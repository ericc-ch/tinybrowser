...
Status: accepted. Keeps [ADR 0007](0007-engine-charter.md) crate graph, Tokio `rt`+`time`, `spawn_blocking`, and size/lint bounds. The `cdp` adapter is a peer crate that depends on `browser` alone. Extends [ADR 0008](0008-wpt-via-webdriver.md): WebDriver stays the WPT driver. Browser, `PageHandle`, and `NetworkSession` live in [ADR 0010](0010-page-actor-ownership.md).

## Process

There is one daemon per named profile. The implicit profile name is `default`. This daemon is the browser process for now. It hosts one Browser bound to that Profile ([ADR 0010](0010-page-actor-ownership.md)).
...
Support the browser WebSocket and direct page WebSockets. On the browser socket, `Target.attachToTarget` with `flatten` true returns a `sessionId`. Later target commands and events carry `sessionId` at the top JSON level. Direct page sockets need no `sessionId`. Internally both route to the same `PageHandle`.

CDP allows multiple client attachments. Page actors serialize commands. Concurrent controllers can still interfere. Do not promise transactional isolation.
...
Classic WebDriver is a peer adapter over `BrowserHandle` and the selected persistent profile. It does not own Browser, Page, Profile, or NetworkSession. A WebDriver session owns only its current browsing context, timeouts, element references, and input state.
...
- The `cdp` crate depends on `browser` alone. Root hosts it beside `webdriver`; the protocol crates do not depend on each other ([ADR 0007](0007-engine-charter.md)).
