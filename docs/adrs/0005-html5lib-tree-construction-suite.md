...
- **Committed snapshot:** reproducible offline, but ~2 MB in git history. A submodule pins the SHA and keeps the bytes out; clones need `git submodule update --init` (harness fails loudly with that instruction). Pin details live in [VENDORED.md](../../third_party/VENDORED.md).
