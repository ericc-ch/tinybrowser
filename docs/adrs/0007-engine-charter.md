...
Crate slogans, a fake `js` seam, and parking DOM/language on `net` were fighting the product: a small page engine with a task loop, not a web-server org chart. The decided shape is three deep crates, a zero-dep inbound HTTP/1.1 helper for protocol adapters, tasks on a Tokio current-thread waiter, and `browser` holding `parse_html`. Browser owns `NetworkSession` ([ADR 0010](0010-page-actor-ownership.md)).
...
- **Tokio `full` / axum / hyper:** server stack. Rejected here for size and for the wrong product; axum/hyper later entered the browser-process protocol adapters only ([ADR 0012](0012-host-protocol-and-cli-stack.md)).
