...
## Milestone: locale-filtered ICU4X Intl slice (2026-09-12)

This non-conformant spike adds an initial `Intl.NumberFormat` and
`Intl.DateTimeFormat`
backed by ICU4X 2.3 and a locale-filtered blob for `en-US`, `es-ES`, `de-DE`,
`ja-JP`, `fr-FR`, `zh-CN`, and `ko-KR`. Number formatting covers decimal,
percent, and currency happy paths; currency fraction overrides, BigInt, and
several newer number options remain unsupported. Date/time formatting is
limited to the Gregorian calendar and UTC, and maps component options to the
nearest available style. The façade is enough to validate the QuickJS bridge,
ICU data strategy, and size—not to claim ECMA-402 conformance.
...
| --- | ---: | ---: | ---: | ---: |
| CLI (`target/release/tinybrowser`) | 5,357,600 | 5,737,376 | **+379,776** | 4,262,624 |

The generated postcard blob is 114,220 bytes. Marker-based generation keeps
unused ICU datasets out of the executable; `tools/intl/generate-data` records
the locked locale list and regeneration command. This payload was generated by
`icu4x-datagen` 2.3.0 from CLDR 48.2.1 and UCD 17 data.

Six focused vendored Test262 cases pass through the shipping CLI: NumberFormat
defaults, bound formatting, non-finite values, and fraction digits; plus
DateTimeFormat bound formatting and non-finite rejection. These are smoke
results for the implemented paths, not a substitute for a future full
`intl402` conformance run.
