...
- **`:lang()`** reads the literal `lang` attribute only. `xml:lang` and `Content-Language` land with `net`.
...
- **`:scope`** under document queries is the document element. Element-rooted scope context lands with js.
