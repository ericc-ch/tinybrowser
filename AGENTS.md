...
- The executable owns one multi-thread Tokio runtime for the browser process. `BrowserHandle` and `TabHandle` are async-only. CDP and WebDriver are async adapters on that runtime. Outbound HTTP and WebSocket dials use hyper-util, native-tls (system OpenSSL, dynamically linked), and tokio-tungstenite in `net`. HTTP server crates stay in browser-process protocol adapters (axum, per [ADR 0020](docs/adrs/0020-server-stack.md)), and clap stays in the CLI. Do not add Tokio `full`, smol, a multi-thread renderer runtime, or an HTTP stack to `renderer` ([ADR 0019](docs/adrs/0019-async-browser-runtime-and-io.md)). Size numbers: [`docs/researches/size-budget.md`](docs/researches/size-budget.md).
...
- Project notes live in the Obsidian vault at `~/Documents/obsidian/everything/projects/tinybrowser/`, not in this repository: `v2-migration.md` is the migration ledger and decisions, `handoff.md` the current session state, and the other notes hold research, walkthroughs, and reviews.
