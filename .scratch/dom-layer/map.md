...
Destination: `crates/dom` has a decided scope + API surface, specced and ready to build as the first layer of tinybrowser. Later layers (`net`, `js`, `browser`) get their own wayfinding sessions once this ships.
...
- Stack already decided (docs/size-budget.md): html5ever 0.39 tree builder, selectors 0.26 later (~75 KB).
- WebIDL verification strategy exists (docs/webidl.md) but is CI-side; doesn't block dom design.
- Layer-by-layer, not vertical slice. Deps go downward only; `dom` depends on nothing.
...
- [TreeSink above dom](./tickets/04-treesink-lives-above-dom.md): dom stays zero-dep; html5ever adapter lives higher up.
- [Selectors in v1](./tickets/05-selectors-in-v1.md): querySelector-style matching ships with the first release (+~75 KB).
- [Send, not Sync](./tickets/06-send-not-sync.md): hand-off between workers legal; concurrent access compiler-forbidden; zero locks.
- [Testing strategy](./tickets/07-testing-strategy.md): vendored html5lib tree-construction suite + unit tests for arena/spill/selectors.
- [Size checkpoint](./tickets/08-size-checkpoint.md): probe binary exercising parse+selectors; marginal recorded in docs/size-budget.md.
